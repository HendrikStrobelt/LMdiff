<template>
  <div class="InteractiveTokens">
    <div class="token" v-for="(t, index) in tokens" :ref="addTokenRef"
         ...
         borderTop:'1px solid',
         borderTopColor:(index===showHoverFor)?'gray':'rgba(0,0,0,0)',
         }"
    ...
    >
      <div class="tt-c" v-show="!!currentTokenInfo?.m1">
        ...
        > {{ token_cleanup(topk[0]) }} - {{ formatNumbers(topk[1]) }}
        </div>
      ...
      <div class="tt-c" v-show="!!currentTokenInfo?.diff">
        <div>&Delta;Prob: <span
            :style="{color:conditionalColor(currentTokenInfo?.diff?.prob)}">
